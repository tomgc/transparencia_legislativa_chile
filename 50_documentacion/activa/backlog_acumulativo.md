...
| 17 | v17 | 2 | Opus 5 | captura cruda del 36 y rescate del nodo Votaciones |
| 18 | v18 | 3 | Opus 5 | contrato temporal de la captura + reparacion del bot semanal |
...
### Sesion 17 (v17) — captura cruda del 36 y rescate del nodo Votaciones

#### Extraccion de datos

**51. Captura genuinamente cruda para el paso 36 y rescate del nodo `Votaciones`.**
Categoria: extraccion de datos. El pendiente P-63 estaba caracterizado como trabajo
barato: conservar un nodo que `retornarProyectoLey` ya entrega y que el parser
descartaba, con cero llamadas nuevas. La lectura de los dos archivos refuto la
premisa antes de escribir codigo: `con_cache()` persiste el retorno de la funcion de
descarga (`10_utils.R:228-240`) y en el 36 ese retorno ya es el tibble parseado
(`36:69-104`), de modo que el XML nunca tocaba disco y la carpeta de dato crudo
contenia un derivado del parser. La consecuencia de fondo no era el costo sino que
la guarda de autorregeneracion instalada en la sesion 16 prometia regenerar sin red
algo que, para el paso 36, solo podia reproducir con los campos que ese parser habia
conservado. El titular opto por arreglar la premisa antes de usarla, al mismo precio
que ignorarla. Se ejecuto un encargo autonomo con seis compuertas de precondicion,
doce criterios contrastables y panel adversarial de cuatro verificadores con codigo
propio. El 36 pasa a persistir el XML como texto bajo clave nueva
(`detalle_proyectos_xml_<anio>`) y a derivar el tibble desde esa captura; la captura
anterior quedo intacta en 43 de 43 md5. El nodo resulto tener 14 campos reales y no
los 4 hipotetizados, seis de ellos con atributo de dominio que se conservan con
codigo y glosa. `proyectos_detalle.rds` paso de 5 a 20 columnas manteniendo una fila
por boletin. Cobertura medida: 115 de 115 boletines votados con nodo no vacio, 0 de
266 solo autorados, 723 nodos de votacion, `articulo` no vacio en 619 de 723. La
captura pesa 129,39 K, el 6,3 % del umbral fijado en el encargo. El artefacto
publicado no cambio: 156 de 156 archivos identicos excluido `metadatos.generado`, 0
claves nuevas en el JSON. Resuelve el pendiente P-63 y genera el pendiente P-74.
Verificado con 12 de 12 criterios y panel de 4 de 4 sin hallazgos. Publicado en el
PR #7, mergeado a `main`.

#### Medicion y exploracion de fuentes

**52. Medicion de la llave directa voto -> proyecto, insumo de P-66.**
Categoria: diagnostico/exploracion. El ejecutor reporto que 546 de los 723 nodos de
votacion estaban presentes en `votos.rds` y 245 solo en `votos.rds`; esas dos cifras
coincidian exactamente con el denominador de la decision D26 y su complemento, lo
que era demasiado redondo para ser casualidad, y se pidio medirlo desde el otro
lado. Resulto identidad de conjuntos y no coincidencia: de las filas de `votos.rds`
con boletin no nulo, 84 630 de 84 630 tienen su `votacion_id` en el nodo; de las que
tienen boletin nulo, 0 de 37 975. Por votacion distinta, 546 de 546 de tipo
`Proyecto de Ley` y 0 de 245 del resto (`Otros` 0 de 117, `Proyecto de Resolucion` 0
de 105, `Proyecto de Acuerdo` 0 de 23), con `setequal()` verdadero en ambos
sentidos, porque las votaciones de acuerdo y resolucion no cuelgan de ningun
boletin. El boletin obtenido por llave coincide con el extraido por expresion
regular en 546 de 546, sin discrepancias: el cambio no agregaria cobertura, elimina
la dependencia del formato del texto libre, que es la zona fragil 2 declarada en
v16. La medicion tambien explico los 177 nodos ausentes de `votos.rds` (176 de anos
anteriores a 2026 y 1 del 2026-08-04, un dia despues del corte), y ese ultimo dato
es el que origina P-74. No se implemento nada: por invariante del encargo, la
medicion es insumo de P-66 y no cambio de producto.

### Sesion 18 (v18) — contrato temporal de la captura y reparacion del bot semanal

**53. Medicion del contrato temporal del nodo `Votaciones` (P-74 acto a).**
Medicion read-only en tres fases, sin escrituras fuera de andamios. Encontro 1 de 723
eventos posteriores al corte declarado, en 1 de 115 boletines, y 0 consumidores del
nodo en codigo y en las 106 claves de los 156 JSON. Su hallazgo decisivo no fue la
cifra sino la causa: la captura del paso 36 se descargo el 2026-08-08 bajo una clave
que declara el corte 2026-08-03, mientras las otras siete capturas del mismo corte se
bajaron el dia que la clave dice. Eso movio el problema desde el sello (A67) hacia la
ventana entre descarga y corte declarado, y abrio la via que ninguno de los dos
caminos previstos cubria. Dos afirmaciones del primer borrador, una de ellas
causalmente falsa, fueron derribadas por el panel adversarial y reemplazadas por
medicion, no por texto mas suave.

**54. Contrato temporal de la captura: guarda, registro y reporte, mas contencion en
el 36 (P-74 acto b, decision D31).** El pipeline se detiene antes de escribir una
captura cuya descarga caiga fuera del corte que su clave declara, con escape
explicito de un solo uso; cada captura nueva persiste su fecha real de descarga; y
`run_all()` reporta al cerrar en tres estados (`dentro_de_corte`, `fuera_de_corte`,
`sin_registro`) con denominador. Como contencion, el paso 36 acota el nodo al corte y
emite cuantos eventos descarta sobre cuantos. Las capturas anteriores al contrato no
se reescriben (D24) y quedan en `sin_registro`, que no es conformidad. Verificado con
14 criterios y neutralidad del artefacto publico en ambos destinos.

**55. Reparacion de la guarda circular que tenia roto el refresh semanal (P-65).**
La guarda de autorregeneracion exigia capturas crudas del corte vigente para permitir
continuar, y en el runner esas capturas son exactamente las que la corrida iba a
descargar: circular en el unico camino capaz de romper el ciclo. La corrida
programada del 2026-08-10 ya habia fallado en produccion con 0 de 6 pasos. Se
reparo separando tres estados que la guarda colapsaba en uno, decidiendo la rama de
primera corrida por existencia de archivos en disco y no por `is.na()` del sello (que
confunde ausencia con corrupcion), y unificando la resolucion de corte de
`capturas_crudas_de_paso()`. Verificado en produccion por `workflow_dispatch`: 10 de
10 pasos, y el PR del bot mergeado.

### Sesion 19 (v19) — cierre de los escapes de la guarda de arranque y locale UTF-8

**56. Los dos huecos de la guarda de arranque, cerrados en un solo PR (P-76 y
P-77, decision D33).** El escape `camara.permitir_descarga_inicial` pasa a ser de
un solo uso, simetrico con el del contrato temporal; y la guarda distingue por
primera vez el arranque legitimo del borrado de intermedios, mediante un rastro
propio que ella misma escribe. El diseno que el encargo proponia murio en su
propia compuerta: `.gitkeep` esta trackeado, asi que el directorio de intermedios
existe en todo checkout y `dir.exists()` no discrimina nada; implementarlo habria
detenido al runner en cada corrida. La medicion tambien descarto destrackear
`.gitkeep`, porque la ruta de recuperacion que la propia guarda imprime corre
extractores sueltos y `escribir_atomico()` no crea directorios. Incluye la
correccion de la primera linea del `stop()`, que contaba como desalineados a
archivos inexistentes. Verificado con 14 criterios, cero llamadas de red y 9 de 9
funciones protegidas identicas a `HEAD`.

**57. D31 materializada como archivo de decision (P-78).** La decision operaba en
el codigo desde el PR #8 pero vivia solo en el traspaso. El archivo registra que
afirma y que NO afirma una captura sobre su alcance temporal, las dos
alternativas descartadas, la tension con A67, las siete piezas de la
implementacion y los dos pendientes que abre (P-79, P-81). En la misma sesion se
resolvio descartar las 6 capturas del corte 2026-08-09 que quedaron en cuarentena
fuera del repositorio (P-80).

**58. Guarda de locale UTF-8 en cuatro puntos de arranque (P-59, decisiones D34 y
D35).** Apaga el gatillo 4ter, encendido desde dos aperturas. La instalacion no
fue mecanica: el punto obvio (`10_configuracion.R`) cubre 19 de 28 archivos y
deja fuera al escaner y al camino cuyo texto termina en el mensaje de commit y en
el cuerpo del PR del bot, es decir, en el historial publico; y el punto mas ancho
(`10_utils.R`) quedo descartado porque resolver la ruta del helper desde ahi
exige un paquete en el archivo que define `instalar_si_falta()`, cargado tres
lineas antes de que esa funcion corra. La cobertura se declara sobre el universo
alcanzable por cierre transitivo desde tres raices (13 de 30 archivos), con los
cuatro reproductores congelados de la auditoria 2026-08-07 excluidos y nombrados.
Verificado con 9 criterios, incluida la prueba de la guarda en subproceso con la
locale forzada en las dos direcciones.

- **59.** Entrada de P-59 (guarda de locale UTF-8) en `CLAUDE.md`, desbloqueada por
  el merge del PR #11: la lista de "Ultimos cambios" incorpora P-59 como cambio 1,
  la entrada de P-76/P-77 declara su merge, y `10_utils/10_locale.R` se suma a la
  estructura de archivos relevantes. Cierra P-83. Commit `0ecb44e` en `main`.
  Categoria: documentacion.

- **60.** Sondeo de LeyChile y `datos.bcn.cl` como fuentes tematicas alternativas
  (P-68), con encargo, reproductor, manifiesto de 43 llamadas y veredicto.
  Resultado: NO. LeyChile cerrada por techo medido (18 de 422 llego a ser ley; 4
  de 336 en cohorte 2026); `datos.bcn.cl` cerrada por control positivo fallido (3
  de 5) y por cobertura 0 de 422 medida por los portadores correctos. Confirma la
  hipotesis B del veredicto del eje tematico desde el otro lado y acota P-66.
...
- **v17:** 2 entradas nuevas (51-52). Conteos actualizados (extraccion de datos
  6->7, diagnostico/exploracion 10->11); suma de la columna 51->53 y porcentajes
  recalculados sobre 53 (suman 100,1 por redondeo a un decimal; recuento
  programatico de esta sesion, `conteo_cierre_v17.R`). Sin categorias nuevas. Sin
  renumeracion ni reescritura de entradas 1-50. La discrepancia heredada sigue
  abierta con el mismo signo y la misma unidad (columna 53, entradas 52).
  **Mapeo declarado:** la entrada 51 va a `extraccion de datos` y no a
  `infraestructura`, pese a que su mitad mas estructural es el rediseno de la
  captura, porque la intencion primaria del titular fue obtener un dato que se
  estaba descartando; el rediseno fue el medio que esa obtencion exigio, no el fin
  (criterio inverso al aplicado a las entradas 49 y 50 de v16, cuyo objeto era la
  reproducibilidad y no el dato). La entrada 52 se cuenta pese a no haber sido una
  solicitud literal del titular: nacio de una sospecha que el titular planteo
  explicitamente sobre dos cifras del reporte, produjo un veredicto propio y cambia
  el diseno de P-66, mismo criterio con que v16 conto la entrada 48. Se declara
  aqui por si una sesion futura juzga que el criterio fue laxo. **Lo que NO se
  conto, declarado para que una sesion futura no lo duplique:** el merge del PR #7
  es la publicacion del artefacto de la entrada 51, mismo criterio con que v16
  excluyo los merges de los PRs #5 y #6; la correccion del comentario obsoleto de
  `10_utils.R:403` y la del chequeo de neutralidad que dio falso positivo son
  higiene sobre artefactos de la propia sesion; la verificacion del estado de `git`
  que disolvio la contradiccion entre `f1584b8` y `2cd04cd` es lectura de estado, no
  cambio de producto; y el push del cierre y los commits de documentacion son
  operaciones de git (precedente de la sesion 9).


- **v18:** 3 entradas nuevas (53-55). Conteos actualizados (infraestructura 6->7,
  diagnostico/exploracion 11->12, automatizacion 3->4); suma de la columna 53->56 y
  porcentajes recalculados sobre 56 (recuento programatico en R exigido por el
  paquete de cierre, con parada si difiere de lo declarado). Sin categorias nuevas.
  Sin renumeracion ni reescritura de entradas 1-52. La discrepancia heredada sigue
  abierta con el mismo signo y la misma unidad (columna 56, entradas 55).
  **Mapeo declarado:** la entrada 54 va a `infraestructura` y no a `extraccion de
  datos` pese a tocar el paso 36, porque su intencion primaria fue fijar una
  propiedad del sistema (que una clave no pueda mentir sobre su dato) y no obtener
  dato nuevo; criterio inverso al aplicado a la entrada 51 de v17. La entrada 55 va a
  `automatizacion` y no a `infraestructura` porque el objeto reparado es el unico
  camino desatendido del proyecto y su intencion primaria fue restituir el refresh
  automatico, aunque el arreglo viva en `10_utils.R`. **Lo que NO se conto, declarado
  para que una sesion futura no lo duplique:** el cierre retroactivo de la sesion 17
  (archivado del v16, commit del traspaso y correccion de dos entradas de
  `CLAUDE.md`) son operaciones de git y documentacion interna, precedente de la
  sesion 9 y del delta de v17; los merges de los PRs #8, #9 y #10 son la publicacion
  de los artefactos de las entradas 54 y 55, mismo criterio con que v16 y v17
  excluyeron los merges de los PRs #5, #6 y #7; la corrida por `workflow_dispatch` es
  la verificacion de la entrada 55, no un cambio de producto; y la descarga no
  solicitada de 3230 llamadas es un error del asistente, que por la nota metodologica
  no cuenta y queda registrado en el §15 del traspaso.

- **v19:** 3 entradas nuevas (56-58). Conteos actualizados (infraestructura 7->8,
  documentacion 7->8, automatizacion 4->5); suma de la columna 56->59 y
  porcentajes recalculados sobre 59 (recuento programatico en R exigido por el
  paquete de cierre, con parada si difiere de lo declarado). Sin categorias
  nuevas. Sin renumeracion ni reescritura de entradas 1-55. La discrepancia
  heredada sigue abierta con el mismo signo y la misma unidad (columna 59,
  entradas 58). **Mapeo declarado:** la entrada 56 va a `automatizacion` y no a
  `infraestructura`, mismo criterio con que v18 clasifico la entrada 55: el objeto
  reparado es el camino desatendido del proyecto, aunque el arreglo viva en
  `10_utils.R`. La entrada 58 va a `infraestructura` y no a `documentacion` pese a
  producir un marcador, porque su intencion primaria fue fijar una propiedad del
  entorno de ejecucion. Las tres decisiones de diseno de la sesion (D33, D34, D35)
  no se cuentan aparte: son el contenido de las entradas 56 y 58, no cambios
  distinguibles. **Lo que NO se conto, declarado para que una sesion futura no lo
  duplique:** la correccion de la primera linea del `stop()` es parte de la entrada
  56, no un cambio propio, porque nacio del panel adversarial de ese mismo PR y
  vive en su diff; el descarte de las 6 capturas en cuarentena (P-80) es una
  decision sobre archivos que nunca estuvieron en el repositorio y se registra en
  la entrada 57; los dos encargos, los dos arneses y los dos logs son instrumentos
  de las entradas 56 y 58; y los PRs #11 y #12 quedaron abiertos al cierre, asi que
  su merge, cuando ocurra, es la publicacion de artefactos ya contados y no cuenta,
  mismo criterio con que v16, v17 y v18 excluyeron los merges de los PRs #5 a #10.
- **v20:** 2 entradas nuevas (59-60). Conteos actualizados
  (diagnostico/exploracion 12->13, documentacion 8->9); suma de la columna 59->61 y
  porcentajes recalculados sobre 61 (recuento programatico en R exigido por el
  paquete de cierre, con parada si difiere de lo declarado). Sin categorias nuevas.
  Sin renumeracion ni reescritura de entradas 1-58. La discrepancia heredada sigue
  abierta con el mismo signo y la misma unidad (columna 61, entradas 60). Mapeo
  declarado y exclusiones: ver el bloque BACKLOG_DELTA del paquete de cierre v20,
  reproducido aqui en lo esencial: la entrada 60 va a `diagnostico/exploracion` y
  no a `extraccion de datos` porque su intencion primaria fue determinar si una
  fuente sirve, no obtener dato para el producto, mismo criterio con que v16
  clasifico la auditoria de fuentes Camara/Senado (P-61). NO se contaron: el merge
  de los PRs #11 y #12 (v19 ya los declaro como publicacion de artefactos
  contados); la higiene del stash y su preservacion como rama `wip-sesion6`; los
